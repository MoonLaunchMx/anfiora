...
export default function SupplierDetailModal({
  ...
  const openEditPaymentForm = (p: SupplierPayment) => {
    ...
    setPayMethod(p.payment_method ?? 'transferencia')
    setPayBy(p.paid_by ?? 'pareja')
    ...
    <>
      <AnimatePresence>
        ...
        >
          <motion.div
            ...
            className="flex max-h-[95vh] w-full flex-col rounded-t-2xl bg-white shadow-xl md:max-h-[90vh] md:max-w-3xl md:rounded-2xl"
          ...
          >
            ...
            <div className="flex flex-shrink-0 items-start justify-between gap-4 border-b border-[#e8e8e8] px-5 py-4 md:px-6">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="mt-1 truncate text-lg font-semibold text-[#1D1E20] md:text-xl">
                  {name || 'Sin nombre'}
                </h2>
              </div>
              <button
            ...
            {/* CONTENT */}
            <div className="flex-1 space-y-7 overflow-y-auto px-5 py-5 md:px-6">
      ...
      {showReviewModal && (
        <SupplierReviewModal
          ...
          initialReview={item.review_text}
          initialMood={item.mood}
          initialSpeed={item.response_speed}
          onSaved={handleReviewSaved}
          onSkip={handleReviewSkipped}
        />
      )}
    </>
  )
