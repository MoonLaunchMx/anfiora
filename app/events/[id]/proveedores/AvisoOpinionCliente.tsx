        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          {canAdmin && info.estado !== 'sin_pedir' && (
            <div ref={menuRef} className="relative">
              ...
              {menu && (
                <div className="absolute right-0 z-20 mt-1.5 w-40 rounded-xl border border-[#e0e0e0] bg-white p-1.5 shadow-lg">
                  {[7, 14, 30].map(dias => (
                    <button
                      key={dias}
                      type="button"
                      disabled={ocupado}
                      onClick={() => masDias(dias)}
                      className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[#f5f5f5] disabled:opacity-50"
                    >
                      {dias} días más
                    </button>
                  ))}
