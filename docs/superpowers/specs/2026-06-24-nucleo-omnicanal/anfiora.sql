...
-- El historial de mensajes es valioso y un hilo puede ser COMPARTIDO (un mismo
-- proveedor sirve a varios eventos). Por eso TODO es SET NULL: al borrar el
-- evento/invitado/proveedor, la conversacion se DESVINCULA y cae al inbox
-- general, pero su historial sobrevive. Esto cierra el agujero de "perdi el
-- historial de un evento" al reasignar o borrar.
alter table conversations
...
-- (prospecto sin clasificar). Lo que se prohibe es invitado Y proveedor a la vez.
alter table conversations
...
--                        (asignado o no), via workspace_id = auth.uid().
--   - Inbox de evento -> un COLABORADOR ve solo conversaciones de SU evento:
--                        las de invitados (tenant_id) y las de proveedores
--                        ligados a ese evento via event_suppliers.
--   - Sin clasificar (tenant NULL, sin proveedor) -> solo el dueno.
--
-- NOTA proveedores multi-evento: la pertenencia proveedor<->evento NO se decide
-- por tenant_id (seria fragil: un hilo, varios eventos). Se decide por
-- event_suppliers (muchos-a-muchos, persistente). Por eso un hilo de proveedor
-- aparece en el inbox de CUALQUIER evento que lo tenga contratado, sin mover su
-- tenant_id.
--
-- (auth.uid() = users.id en Anfiora.)
...
create policy cv_collaborator on conversations for select
  using (
    ...
    or contact_supplier_id in (
      ...
      where ec.user_id = auth.uid() and ec.status = 'accepted'
    )
  );
